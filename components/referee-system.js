...
function boot(){
  ...
  function followTarget(f,b,dt,t){const center=(f.left+f.right)/2,margin=f.w*.11;if(t-ref.lastSideChange>850){if(ref.followSide===1&&b.x>center+margin){ref.followSide=-1;ref.lastSideChange=t}else if(ref.followSide===-1&&b.x<center-margin){ref.followSide=1;ref.lastSideChange=t}}let attackDir=0;if(b.owner)attackDir=b.owner.team==='blue'?1:-1;else if(Math.abs(b.vx)>35)attackDir=Math.sign(b.vx);if(!attackDir)attackDir=b.x<center?1:-1;const behind=78,lateral=66;let desiredX=b.x-attackDir*behind,desiredY=b.y+ref.followSide*lateral;const adjusted=crowdAdjustedTarget(desiredX,desiredY,f);desiredX=adjusted.x;desiredY=adjusted.y;if(ref.followX==null||ref.followY==null){ref.followX=desiredX;ref.followY=desiredY}const alpha=1-Math.exp(-Math.max(.001,dt)/.28);ref.followX+=(desiredX-ref.followX)*alpha;ref.followY+=(desiredY-ref.followY)*alpha;return{x:ref.followX,y:ref.followY}}
