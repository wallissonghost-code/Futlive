...
function boot(){
  ...
  const ref={state:STATES.FOLLOWING,x:e.field().w*.5,y:e.field().h*.62,target:null,busy:false,discipline:new Map(),STATES,sprite,ballContactAt:0,ballIgnoreUntil:0,followX:null,followY:null,followSide:1,lastSideChange:0};
  ...
  function ballContact(){
    const b=e.ball,phase=window.FutLiveMatchState?.phase,now=performance.now();
    if(b.owner||b.type==='foul-dead'||!['PLAYING','KICKOFF'].includes(phase)||now<ref.ballIgnoreUntil||now-ref.ballContactAt<420)return;
    const speed=Math.hypot(b.vx||0,b.vy||0);if(speed<28)return;
    const hb=sprite.getBallHitbox(ref.x,ref.y),dx=b.x-hb.x,dy=b.y-hb.y,n=(dx*dx)/(hb.rx*hb.rx)+(dy*dy)/(hb.ry*hb.ry);if(n>1)return;
    const centerFactor=clamp(1-n,0,1),speedPenalty=clamp((speed-120)/300,0,1)*.18;
    const hitChance=clamp(.12+centerFactor*.70-speedPenalty,.10,.82);
    if(Math.random()>hitChance){ref.ballIgnoreUntil=now+190;window.dispatchEvent(new CustomEvent('futlive:referee-ball-miss',{detail:{chance:hitChance,centerFactor,speed}}));return}
    const mag=Math.hypot(dx,dy)||1,nx=dx/mag,ny=dy/mag,dot=b.vx*nx+b.vy*ny;
    const strength=.65+Math.random()*.55,keep=.70+Math.random()*.16;
    let vx=b.vx-(1.05+strength*.55)*dot*nx,vy=b.vy-(1.05+strength*.55)*dot*ny;
    vx=vx*keep+nx*(10+Math.random()*18);vy=vy*keep+ny*(10+Math.random()*18);
    b.vx=vx;b.vy=vy;b.x+=nx*(3+Math.random()*3);b.y+=ny*(3+Math.random()*3);ref.ballContactAt=now;ref.ballIgnoreUntil=now+260;
    window.dispatchEvent(new CustomEvent('futlive:referee-ball-contact',{detail:{x:b.x,y:b.y,beforeSpeed:speed,afterSpeed:Math.hypot(vx,vy),chance:hitChance,centerFactor,ballType:b.type,referee:{x:ref.x,y:ref.y}}}))
  }
