...
function boot(){
  ...
  if(!e||!stage||!pauseBtn||!game||!e.players?.length){setTimeout(boot,45);return}if(window.FutLiveReferee)return;
  const el=document.createElement('div');el.className='referee-agent';el.textContent='🧑‍⚖️';stage.appendChild(el);
  const notice=document.createElement('div');notice.className='referee-notice';document.body.appendChild(notice);
  const style=document.createElement('style');style.textContent=`.referee-agent{position:absolute;z-index:5;width:30px;height:30px;display:grid;place-items:center;font-size:22px;transform:translate(-50%,-50%);filter:drop-shadow(0 3px 4px #0008);pointer-events:none}.referee-notice{position:fixed;z-index:110;left:50%;top:29%;transform:translate(-50%,-50%) scale(.85);opacity:0;pointer-events:none;padding:9px 15px;border-radius:999px;background:#071019e8;border:1px solid #ffffff2d;color:#fff;font-size:12px;font-weight:1000;letter-spacing:.08em;transition:.16s}.referee-notice.show{opacity:1;transform:translate(-50%,-50%) scale(1)}`;document.head.appendChild(style);
  const ref={state:STATES.FOLLOWING,x:e.field().w*.5,y:e.field().h*.62,target:null,busy:false,discipline:new Map(),STATES};
  function setState(s){ref.state=s;el.dataset.state=s}
  ...
  let last=performance.now();function loop(t){const dt=Math.min(.04,(t-last)/1000||.016);last=t;const f=e.field();let tx,ty;if(ref.target&&ref.state!==STATES.FOLLOWING){tx=ref.target.x;ty=ref.target.y}else{const b=e.ball;tx=clamp(b.x+(b.x<f.w*.5?55:-55),f.left+40,f.right-40);ty=clamp(b.y+(b.y<f.h*.5?55:-55),f.top+35,f.bottom-45)}const dx=tx-ref.x,dy=ty-ref.y,m=Math.hypot(dx,dy);if(m>2){const s=Math.min(m,92*dt);ref.x+=dx/m*s;ref.y+=dy/m*s}el.style.left=ref.x+'px';el.style.top=ref.y+'px';requestAnimationFrame(loop)}requestAnimationFrame(loop);
