...
function boot(){
  ...
  if(e.__matchContextV056)return;e.__matchContextV056=true;
  ...
  function derive(team,f){
    ...
    const c=context[team];Object.assign(c,{team,stage:st,diff,mentality,risk,tempo,line,press,verticality,width,strongSide,weakSide,possession:share,updatedAt:performance.now()});
  ...
  function weakSideBonus(team,p,f){const c=context[team];if(!c)return 0;const isTop=p.y<f.h*.5,isWeak=(c.weakSide==='TOP'&&isTop)||(c.weakSide==='BOTTOM'&&!isTop);return isWeak?18:0}
  ...
  window.FutLiveMatchContextAI={version:VERSION,context,possession,get:team=>({...context[team]}),stage,debug:()=>({stage:stage(),elapsedMs:elapsed(),blue:{...context.blue},red:{...context.red},possession:{blue:possession.blue,red:possession.red,shareBlue:possessionShare('blue')}})};
}
boot();
})();