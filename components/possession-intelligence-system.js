...
function boot(){
  ...
  if(e.__possessionIntelligenceV062)return;e.__possessionIntelligenceV062=true;
  ...
  // Cada jogador recebe uma faixa própria. O alvo nunca nasce simplesmente em cima do portador.
  function tacticalSlot(p,c,f,index){
    const a=attack(p.team),homeY=f.h*(p.home?.[1]??.5),homeX=f.w*(p.home?.[0]??.5),span=f.bottom-f.top;
    const upper=homeY<f.h*.5,side=upper?-1:1;
    const carrierProgress=p.team==='blue'?(c.x-f.left)/(f.right-f.left):(f.right-c.x)/(f.right-f.left);
    let depth=58,laneY=homeY;
    if(p.personality==='finisher'){depth=carrierProgress>.68?84:118;laneY=clamp(c.y+side*span*.10,f.top+44,f.bottom-54)}
    else if(p.personality==='wing'){depth=carrierProgress>.72?62:104;laneY=clamp(homeY+side*span*.055,f.top+38,f.bottom-48)}
    else if(p.personality==='support'){depth=44;laneY=clamp(c.y+side*span*.23,f.top+42,f.bottom-52)}
    else {depth=-52;laneY=clamp(homeY*.58+c.y*.42,f.top+42,f.bottom-52)}
    // Em vez de clamp colapsar todo mundo na mesma borda, mistura profundidade com a posição-base.
    let desiredX=c.x+a*depth;
    const edgePad=58,edgeL=f.left+edgePad,edgeR=f.right-edgePad;
    if(desiredX<edgeL||desiredX>edgeR){const fallback=homeX+(c.x-f.w*.5)*.10;desiredX=desiredX<edgeL?Math.max(edgeL,fallback):Math.min(edgeR,fallback)}
    // pequeno offset por índice garante destinos distintos até para papéis repetidos
    laneY+=((index%3)-1)*18;
    return{x:clamp(desiredX,edgeL,edgeR),y:clamp(laneY,f.top+40,f.bottom-50)}
  }
  function repelTarget(p,q,team){
    let x=q.x,y=q.y;
    for(const m of fielders(team)){if(m===p)continue;const dx=x-m.x,dy=y-m.y,d=Math.hypot(dx,dy)||1;if(d<58){const push=(58-d)*.72;x+=dx/d*push;y+=dy/d*push}}
    return{x,y}
  }
  function supportRuns(c,dt,f){
    const list=mates(c.team,c).sort((a,b)=>(a.slot??0)-(b.slot??0));
    list.forEach((p,i)=>{let q=tacticalSlot(p,c,f,i);q=repelTarget(p,q,c.team);const minCarrier=p.personality==='creator'?46:62,dCarrier=Math.hypot(q.x-c.x,q.y-c.y);if(dCarrier<minCarrier){const dx=q.x-c.x,dy=q.y-c.y,d=dCarrier||1;q.x=c.x+dx/d*minCarrier;q.y=c.y+dy/d*minCarrier}q.x=clamp(q.x,f.left+48,f.right-48);q.y=clamp(q.y,f.top+40,f.bottom-50);const mul=p.personality==='finisher'||p.personality==='wing'?.82:.66;e.moveToward(p,q.x,q.y,p.speed*mul,dt)})
  }
  function defendWhileOwned(c,dt,f){
    const os=opponents(c.team).sort((a,b)=>e.dist(a,c)-e.dist(b,c));
    os.forEach((p,i)=>{if(i===0){e.moveToward(p,c.x-attack(p.team)*14,c.y,p.speed*.84,dt);return}if(i===1){const side=p.home?.[1]<.5?-1:1;e.moveToward(p,c.x-attack(p.team)*64,c.y+side*42,p.speed*.66,dt);return}const hx=f.w*p.home[0],hy=f.h*p.home[1],shiftX=(c.x-f.w*.5)*.08,shiftY=(c.y-f.h*.5)*.12;e.moveToward(p,clamp(hx+shiftX,f.left+46,f.right-46),clamp(hy+shiftY,f.top+40,f.bottom-50),p.speed*.48,dt)})
  }
  function carrierMove(c,dt,f,pressure){const a=attack(c.team),gy=(f.goalTop+f.goalBottom)/2;let tx=c.x+a*(pressure>=2?24:pressure?34:48),ty=c.y+(gy-c.y)*(pressure?.08:.16);const os=opponents(c.team).filter(o=>e.dist(c,o)<72);if(os.length){let ax=0,ay=0;for(const o of os){const dx=c.x-o.x,dy=c.y-o.y,d=Math.hypot(dx,dy)||1,w=(74-Math.min(74,d))/74;ax+=dx/d*w;ay+=dy/d*w}tx+=ax*(pressure>=2?28:18);ty+=ay*(pressure>=2?34:22)}if(pressure>=3){tx=c.x-a*22;ty=c.y+(c.y<gy?-24:24)}e.moveToward(c,clamp(tx,f.left+34,f.right-34),clamp(ty,f.top+34,f.bottom-44),c.speed*(pressure>=3?.62:pressure>=2?.72:.88),dt);e.syncOwnedBall()}
