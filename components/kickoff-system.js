...
function boot(){
  ...
  function targets(team){const {x:cx,y:cy,f}=center(),other=team==='blue'?'red':'blue',map=new Map(),ownBlue=p=>p.team==='blue';
    ...
    for(const p of e.players){if(p.sentOff)continue;if(p.goalkeeper){map.set(p,{x:p.team==='blue'?f.left+46:f.right-46,y:(f.goalTop+f.goalBottom)/2-27});continue}
      ...
      else if(p===mate)map.set(p,{x:cx+(team==='blue'?-42:42),y:cy+18-27});
  ...
  function begin(team,{initial=false,onStarted=null}={}){if(state.busy)return false;state.busy=true;state.team=team;setPhase(PHASES.SETUP);if(!game.classList.contains('is-paused'))pauseBtn.click();
    ...
    let last=performance.now();function frame(t){if(window.FutLiveMatchState?.phase!==PHASES.SETUP)return;const dt=Math.min(.04,(t-last)/1000||.016);last=t;let max=0;
      ...
      if(max<7||performance.now()-start>2800){ready(setup,onStarted);return}requestAnimationFrame(frame)}requestAnimationFrame(frame);return true}
  function ready(setup,onStarted){setPhase(PHASES.READY);for(const [p,q] of setup.map){p.x=q.x;p.y=q.y;p.ctrl.idle();p.lastDir='idle'}e.ball.owner=null;e.ball.x=setup.cx;e.ball.y=setup.cy;e.ball.vx=e.ball.vy=0;e.ball.type='free';e.paint();setTimeout(()=>kick(setup,onStarted),560)}
  function kick(setup,onStarted){const {taker,mate,cx,cy}=setup;if(!taker||!mate){setPhase('PLAYING');if(game.classList.contains('is-paused'))pauseBtn.click();state.busy=false;return}
