...
function boot(){
  ...
  const teamPlayers=t=>e.players.filter(p=>living(p)&&p.team===t);
  ...
  function secondLastDefenderX(attackingTeam){
    const defs=teamPlayers(other(attackingTeam)).map(p=>p.x).sort((a,b)=>a-b);if(defs.length<2)return null;
    return attackingTeam==='blue'?defs[defs.length-2]:defs[1]
  }
  function offside(attacker,ballX=e.ball.x){
    if(!attacker||attacker.goalkeeper)return false;const f=e.field(),line=secondLastDefenderX(attacker.team);if(line==null)return false;
    const inOppHalf=attacker.team==='blue'?attacker.x>f.w*.5:attacker.x<f.w*.5;if(!inOppHalf)return false;
  ...
  function updateShape(team,f){
    const s=state[team],brain=base.teams[team],players=fielders(team),own=ownGoal(team,f),opp=oppGoal(team,f);
    const defending=e.ball.owner&&e.ball.owner.team!==team;
    ...
    const ys=players.map(p=>p.y),span=ys.length?Math.max(...ys)-Math.min(...ys):0;
  ...
  function assignHybridMarks(team){
    const f=e.field(),s=state[team],defenders=fielders(team),attackers=fielders(other(team));s.marking.clear();
    const used=new Set();for(const d of defenders){let best=null,score=1e9;for(const a of attackers){if(used.has(a))continue;const danger=progress(a.team,a.x,f),dist=e.dist(d,a),central=Math.abs(a.y-f.h*.5),sc=dist-danger*55+central*.04;if(sc<score){score=sc;best=a}}if(best&&score<130){s.marking.set(d,best);used.add(best)}}
  }
  function tacticalTarget(p,tx,ty){
    ...
    let x=tx,y=ty,mult=1;const defending=carrier&&carrier.team!==p.team;
    if(defending){
      ...
      const mates=fielders(p.team).filter(q=>q!==p&&q!==carrier),sameLane=mates.filter(q=>q.aiLane===lane&&Math.abs(q.x-p.x)<72);if(sameLane.length)y+=p.y<f.h*.5?-22:22;
  ...
  e.choosePassTarget=(c)=>{
    if(c?.aiDribbleIntent==='TAKE_ON'&&c.pressureCount<2&&Math.random()<.58)return null;
    ...
    for(const p of cands){const forward=(p.x-c.x)*a,opp=fielders(other(c.team)),space=opp.length?Math.min(...opp.map(o=>e.dist(p,o))):120,dist=e.dist(c,p),s=forward*.26+space*.62-dist*.06+(p.personality==='creator'?10:0)+(p.personality==='finisher'&&forward>0?12:0);if(s>score){score=s;best=p}}
    return best
  };
  e.pass=(c,t)=>{
    if(!c||!t||offside(t,c.x)){const safe=e.choosePassTarget(c);if(!safe||safe===t)return;t=safe}
    const team=c.team,a=attack(team),mates=fielders(team).filter(p=>p!==c&&p!==t),now=performance.now(),third=mates.sort((x,y)=>Math.abs(x.y-t.y)-Math.abs(y.y-t.y))[0]||null;
  ...
  function dribbleBias(c){
    if(!c||c.goalkeeper)return;const f=e.field(),near=fielders(other(c.team)).filter(o=>e.dist(c,o)<62),goalDist=Math.abs(oppGoal(c.team,f)-c.x),score=c.skill.control*.42+c.skill.composure*.20+(c.personality==='wing'?.18:0)+(goalDist<f.w*.28?.10:0)-near.length*.16;
