...
function boot(){
  ...
  let goalLock=false;
  engine.goal=(team)=>{if(goalLock||window.FutLiveMatchState?.phase!=='PLAYING')return;goalLock=true;engine.score[team]++;engine.renderScore();game.dataset.lastGoal=team;engine.ball.x=engine.ball.y=-999;engine.ball.vx=engine.ball.vy=0;engine.ball.owner=null;flash.className='goalFlash '+team+' show';flash.querySelector('.goalTeam').textContent=team==='blue'?'🔵 GOL DO AZUL':'🔴 GOL DO VERMELHO';flash.querySelector('.goalScore').textContent=engine.score.blue+' × '+engine.score.red;if(!game.classList.contains('is-paused'))pauseBtn.click();
    setTimeout(()=>{flash.classList.remove('show');const kickoff=window.FutLiveKickoffSystem;if(kickoff?.beginAfterGoal){kickoff.beginAfterGoal(team,()=>{goalLock=false})}else{engine.resetBall();if(game.classList.contains('is-paused'))pauseBtn.click();goalLock=false}},2200)};
  const overlay=document.createElement('div');overlay.className='returnOverlay';overlay.innerHTML='<div class="returnCard"><h2>Partida pausada</h2><p>Você saiu do jogo. Quer continuar de onde parou ou reiniciar a partida?</p><div class="returnActions"><button class="restartGame" type="button">REINICIAR</button><button class="continueGame" type="button">CONTINUAR</button></div></div>';document.body.appendChild(overlay);const restart=overlay.querySelector('.restartGame'),cont=overlay.querySelector('.continueGame');let autoPaused=false;
  function pauseForExit(){if(game.classList.contains('is-paused')){autoPaused=false;return}pauseBtn.click();autoPaused=true}function showReturn(){if(autoPaused)overlay.classList.add('show')}document.addEventListener('visibilitychange',()=>{if(document.hidden)pauseForExit();else showReturn()});window.addEventListener('pagehide',pauseForExit);window.addEventListener('pageshow',()=>{if(!document.hidden)showReturn()});
  ...
  restart.addEventListener('click',()=>{engine.score={blue:0,red:0};engine.renderScore();overlay.classList.remove('show');autoPaused=false;goalLock=false;if(window.FutLiveKickoffSystem?.beginInitial){window.FutLiveKickoffSystem.busy=false;window.FutLiveKickoffSystem.beginInitial()}else{engine.resetBall();if(game.classList.contains('is-paused'))pauseBtn.click()}});
