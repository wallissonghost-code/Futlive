...
test('mobile kickoff has finite continuous positions and working clock',async({page})=>{
  ...
  for(const s of samples){
    ...
    for(const p of s.players){
      expect([p.x,p.y,p.vx,p.vy].every(Number.isFinite),`non-finite state ${p.id} @ ${s.phase}`).toBeTruthy();
    }
  }
  const suspicious=[];
  ...
  const before=await page.evaluate(()=>window.FutLiveMatchState.elapsedMs);
  await page.waitForTimeout(1200);
  const after=await page.evaluate(()=>window.FutLiveMatchState.elapsedMs);
  expect(after-before).toBeGreaterThan(700);
